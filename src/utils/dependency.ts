...
const getDependencySources = (sourceName?: string) => {
  return (
    dependencySources.find((depSource) => depSource.name === sourceName) ??
    dependencySources[0]
  )
}
...
export const genLink = (
  ...
  return `${dependencySource.url}${pkgName}${version}${
    isStyle ? stylePath : path
  }`
}
...
export const genVueLink = (version: string) => {
  const compilerSfc = genLink('@vue/compiler-sfc', version)
  const runtimeDom = genLink('@vue/runtime-dom', version)
  return {
    compilerSfc,
    runtimeDom,
  }
...
export const getSupportedVersions = (
  ...
  filterPreRelease?: boolean
) => {
  let versions = $(getVersions(pkg))
  return computed(() => {
    if (versions.length === 0) return []
    let supportedVersions = versions.filter((version) =>
      compare(version, minSupportedVersion, '>=')
    )
